...
public class SunHttpConnector {
	...
	public void start() {
		try {
			...
			HttpHandler versionHandler = new AuthenticationHandler(new VersionHandler());
			HttpHandler operationHandler = new OperationHandler(server);
			httpServer.createContext("/", operationHandler).setAuthenticator(sha);
			httpServer.createContext("/version", versionHandler).setAuthenticator(sha);
